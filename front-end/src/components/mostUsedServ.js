        ...
        <div className={classes.root}>
            <Paper className={classes.paper} elevation={9}>
                <Toolbar className={classes.bar}>
                    <Typography variant="h5">Most Used Services </Typography>
                </Toolbar>
                <Table className={classes.table} size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell align='left'><b>Users Using this Service</b></TableCell>
                    ...
                    <TableBody>
                        {list && list.map(session => (
                            <TableRow key={session.SessionID}>
                                ...
                                <TableCell>{session.Users_using_this_service}</TableCell>
...
export default function MostUsedServices() {
    ...
    const loadData = async () => {
        ...
        const url = 'http://localhost:8765/db/api/servicesUsedByTheMostPeople';
        axios.get(url, {
            headers: {
            }
        }).then(response => {
